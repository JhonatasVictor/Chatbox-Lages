    ...
    hamburgueres: [
...

        {
            ...
            nome: "X-Lages", 
            preco: 30.00,
            descricao: "Pão, hambúrguer artesanal 300g, queijo cheddar e queijo minas e fatias de bacon crocante."
        }
    ],
    acompanhamentos: [
...
function mostrarCardapio() {
    ...
    for (const categoria in cardapio) {
        msg += `<strong>${categoria.charAt(0).toUpperCase() + categoria.slice(1)}:</strong><br>`;
        cardapio[categoria].forEach(item => {
            // Adiciona botão de ver detalhes para hambúrgueres
            const botaoDetalhes = (categoria === 'hamburgueres' && item.descricao) 
                ? `<button onclick="mostrarDescricao(${item.id})" style="background:#2196F3;color:white;border:none;padding:6px 10px;border-radius:4px;cursor:pointer;font-size:0.8em;margin-left:10px;">Ver Detalhes</button>` 
                : '';
            
            msg += `
                <div class="item-cardapio" style="margin-bottom:15px;">
                    ${item.nome} - ${formatarPreco(item.preco)}
                    ${botaoDetalhes}
                    <div style="margin-top:8px;">
                        <button onclick="alterarQuantidade(${item.id}, -1)" style="background:#ff7f39;color:white;border:none;width:25px;height:25px;border-radius:4px;cursor:pointer;">-</button>
                        <span id="qtd-${item.id}" style="margin:0 10px;">0</span>
                        <button onclick="alterarQuantidade(${item.id}, 1)" style="background:#ff7f50;color:white;border:none;width:25px;height:25px;border-radius:4px;cursor:pointer;">+</button>
                    </div>
                    <div id="descricao-${item.id}" style="display:none;font-size:1.1em;color:#666;margin:5px 0;padding:8px;background:#f0f0f0;border-radius:4px;font-weight:400;"></div>
                </div>
            `;
        });
        msg += "<br>";
    ...
    msg += `<button onclick="finalizarSelecao()" style="background:#ff7f50;color:white;border:none;padding:10px 15px;border-radius:8px;cursor:pointer;margin-top:15px;">Finalizar Pedido</button>`;
    etapaPedido = 'cardapio';
    return msg;
}
